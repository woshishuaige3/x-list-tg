...
def summarize_with_gemini(cfg, tweets, daily=False):
    ...
    if daily:
        scope = (
            "下面是我关注的 X(Twitter) 列表里【今天一整天】的推文（投资、AI、加密等主题）。\n"
            "我是做投资的，主要追踪 AI 硬件产业链。请帮我把今天的内容整理成一份当日总结。"
        )
    else:
        scope = (
            "下面是我关注的 X(Twitter) 列表里最近几小时的新推文（投资、AI、加密等主题）。\n"
            "我是做投资的，主要追踪 AI 硬件产业链。请帮我整理成一份简报。"
        )
    ...
    prompt = (
        ...
        "然后另起一行：⟪🔺 基本面变化 🔻⟫\n"
        "再一行：列出本次涉及投资基本面变化的帖子序号（如「本次 3、7 号」）；若本次没有，就写「本次无」。\n"
        "然后空一行。\n"
        "\n"
        "正文按【投资板块】分组，不要按美股/A股/市场来分。主题小标题单独一行并加粗，前面带 emoji。\n"
        "请优先使用下面这套固定板块（只列实际出现的板块，顺序也严格按这个来，宏观永远排最前，主题标题本身不要编号）：\n"
        "⟪🌏 宏观⟫（跨板块大势：利率、汇率、地缘、大盘情绪，不专属某个板块的信息）\n"
        "⟪🏗️ 云厂⟫（所有做云/数据中心的公司的一切动态：资本开支(CapEx)与指引、财报、云业务增速、AI 战略、自研模型、重大客户与合作等。含五大海外云厂（微软/谷歌/亚马逊/Meta/Oracle）、其他海外中小云厂(如 CoreWeave、Nebius 等)、以及中国云厂(阿里云、腾讯云、华为云、字节火山引擎、百度智能云等)。这些云厂即使发布模型，也一律归这里，不进「模型厂商」）\n"
        "⟪🤖 模型厂商⟫（纯做大模型的公司：OpenAI、Anthropic、xAI、Mistral、DeepSeek、月之暗面等的模型发布、能力、定价、融资、战略动向。不含任何自身也做云的公司）\n"
        "⟪💾 存储⟫（HBM、DRAM、NAND、颗粒涨价、原厂产能）\n"
        "⟪🔦 光通信⟫（光模块、光芯片、CPO、800G/1.6T）\n"
        "⟪🏭 半导体设备/材料⟫（先进封装、代工、光刻、上游材料）\n"
        "⟪🧮 算力/AI 芯片⟫（GPU、ASIC、推理需求）\n"
        "⟪🟢 PCB⟫（高多层板、载板、CCL）\n"
        "⟪⚡ 电力/散热⟫（液冷、电源、机柜、数据中心能耗）\n"
        "⟪🪙 加密⟫（加密货币，独立板块，不要归入股票）\n"
        "如果某条帖子明显属于上面清单之外、但对投资很重要的板块（例如机器人、军工、消费电子），"
        "可以自行新开一个板块，用 ⟪🆕 板块名⟫ 的形式，放在固定板块之后。\n"
        "归类优先级：只要涉及任何做云/数据中心的公司（海外大厂、海外中小云厂、中国云厂，含其自研模型如 Gemini），一律归「🏗️ 云厂」；只有不做云的纯大模型公司（OpenAI、Anthropic、xAI、Mistral、DeepSeek 等）才归「🤖 模型厂商」。都不要放进「📎 其他」。\n"
        "实在归不进任何板块的，放到最后的 ⟪📎 其他⟫ 里。\n"
        "\n"
        "每个主题小标题下面，放属于该主题的若干条帖子。每条帖子严格用【三行】，格式如下：\n"
        "  第一行：序号. ⟪一句话精华标题⟫     （序号全局连续，从 1 开始，跨板块继续累加；精华标题加粗，是这条帖子的核心看点。"
        "如果这条帖子涉及投资基本面的变化，就在这一行的最末尾、加粗标题之后加一个标记（不加粗、靠右排）：利好用 🔺、利空用 🔻，例如利好 ⟪xxx⟫ 🔺、利空 ⟪xxx⟫ 🔻）\n"
        ...
        "— @jukan05 «5»\n"
        "\n"
        "关于「基本面变化」的判定（这决定 🔺/🔻 标记和顶部『基本面变化』行）：\n"
        "我说的基本面变化，不是泛泛的利好利空或股价涨跌，而是【结构性变化】，主要看两类：\n"
        "  1) 供需关系的变化：某个环节供不应求/供过于求、涨价或跌价、扩产或砍单、原厂产能与库存的实质变动、大额订单或产能被锁定等。\n"
        "  2) AI 产业链某个环节的核心矛盾（卡脖子/瓶颈）出现或被解决：例如某关键部件从紧缺到缓解、某技术路线取得突破或落地、某个瓶颈环节被打通或恶化。\n"
        "只有满足以上其一、且是新发生的实质变化，才标记并计入顶部『基本面变化』行；单纯的观点、行情点评、股价波动不算。\n"
        "命中后再判断方向：对相关环节/公司是【利好】（供不应求、涨价、扩产被抢、瓶颈缓解、拿到大单等）就标 🔺；是【利空】（供过于求、跌价、砍单、需求转弱、瓶颈恶化、丢单等）就标 🔻。方向不明确时，默认按利好 🔺 处理。\n"
        "通用要求：\n"
        "- 只如实转述发生了什么 / 博主说了什么，不要加你自己的分析点评。\n"
        "- 大白话、简洁。\n"
        "- 必须逐条转述下面列表里的每一条推文，不要遗漏、不要合并、不要自行判断哪条重要而省略。有多少条就输出多少条。\n"
        "- 即使某条内容看起来简短或随意，也要保留并转述。\n"
